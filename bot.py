     # ============================== Imports ==============================
      ...
      def shorten_method(text, lines):
          mapping = [
              ("後からフィードバックの指導を希望する", "後フィードバック"),
              ("後から同時視聴の指導を希望する", "後同時視聴"),
              ("生徒の配信を同時視聴の指導を希望する", "配信同時視聴"),
          ]
      ...
      # ============================== Reaction → Sheet =====================
      async def process_thumbs_up_to_sheet(ws, ts_col_index, name_col_index, status_col_index, mention_map, bot, source_channel_id):
          channel = bot.get_channel(source_channel_id)
          if not channel:
              return
      ...
      class MyBot(commands.Bot):
          ...
          async def update_all(self):
              ...
              if gc and LU_SPREADSHEET_ID:
                  ...
                  mention_map_lu = load_mention_map(gc, LU_MENTION_SPREADSHEET_ID, LU_MENTION_SHEET_NAME)
              ...
              if gc and INC_SPREADSHEET_ID:
                  ...
                  mention_map_inc = load_mention_map(gc, INC_MENTION_SPREADSHEET_ID, INC_MENTION_SHEET_NAME)

              # 1) 下中級生・上級生 → 同一出力チャンネルへEmbed
              list_channel_1 = self.get_channel(LOWER_UPPER_LIST_CHANNEL_ID)
              if list_channel_1:
                  if LOWER_REQUEST_CHANNEL_ID:
                      lower_embed = await create_request_list_embed_for_channel(self, LOWER_REQUEST_CHANNEL_ID, "下中級生 未対応依頼一覧")
                      await upsert_embed(list_channel_1, lower_embed, self.user)
                  if UPPER_REQUEST_CHANNEL_ID:
                      upper_embed = await create_request_list_embed_for_channel(self, UPPER_REQUEST_CHANNEL_ID, "上級生 未対応依頼一覧")
              ...
              if list_channel_2 and INCOLLE_REQUEST_CHANNEL_ID:
                  incolle_embed = await create_request_list_embed_for_channel(self, INCOLLE_REQUEST_CHANNEL_ID, "インカレ生　未対応依頼一覧")
                  await upsert_embed(list_channel_2, incolle_embed, self.user)

              # 3) 👍 リアクションをシートへ反映（系統ごとの列設定を使用）
              if ws_lu:
                  if LOWER_REQUEST_CHANNEL_ID:
                      await process_thumbs_up_to_sheet(ws_lu, LU_TS_COLUMN_INDEX, LU_NAME_COL_INDEX, LU_STATUS_COL_INDEX, mention_map_lu, self, LOWER_REQUEST_CHANNEL_ID)
                  if UPPER_REQUEST_CHANNEL_ID:
                      await process_thumbs_up_to_sheet(ws_lu, LU_TS_COLUMN_INDEX, LU_NAME_COL_INDEX, LU_STATUS_COL_INDEX, mention_map_lu, self, UPPER_REQUEST_CHANNEL_ID)
              if ws_inc and INCOLLE_REQUEST_CHANNEL_ID:
                  await process_thumbs_up_to_sheet(ws_inc, INC_TS_COLUMN_INDEX, INC_NAME_COL_INDEX, INC_STATUS_COL_INDEX, mention_map_inc, self, INCOLLE_REQUEST_CHANNEL_ID)
